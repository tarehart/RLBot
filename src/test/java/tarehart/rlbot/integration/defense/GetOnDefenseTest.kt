...
class GetOnDefenseTest: StateSettingAbstractTest() {

    @Test
    fun testDefenseWithBallOnRight() {
        val testCase = createTestCase(30F)
        runTestCase(testCase)
    }

    @Test
    fun testDefenseWithBallOnLeft() {
        val testCase = createTestCase(-30F)
        runTestCase(testCase)
    }
    ...
    fun createTestCase(ballX: Float): StateSettingTestCase {
        ...
        return StateSettingTestCase(
                ...
                Plan(Posture.OVERRIDE).withStep(GetOnDefenseStep())
                )
    }
}
