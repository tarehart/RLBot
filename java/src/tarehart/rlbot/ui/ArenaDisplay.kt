...
class ArenaDisplay : JPanel() {
    ...
    fun updateInput(input: AgentInput) {
        ...
        realBallColor = input.latestBallTouch.map {
            bt -> if (bt.team === Bot.Team.BLUE) BLUE_BALL_COLOR else ORANGE_BALL_COLOR
    ...
    override fun paintComponent(g: Graphics) {
        ...
        val blueInitiative = if (latestInput.team === Bot.Team.BLUE) friendlyInitiative else enemyInitiative
        val orangeInitiative = if (latestInput.team === Bot.Team.ORANGE) friendlyInitiative else enemyInitiative
    ...
    private fun drawCar(car: CarData, isOurCar: Boolean, hasInitiative: Boolean, g: Graphics2D) {
        // Draw the car
        val c = if (car.team === Bot.Team.BLUE) BLUE_COLOR else ORANGE_COLOR
